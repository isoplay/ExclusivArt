...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'pedidos_observacao_cliente_len'
      AND conrelid = 'public.pedidos'::regclass
  ) THEN
    ALTER TABLE public.pedidos
      ADD CONSTRAINT pedidos_observacao_cliente_len
      CHECK (observacao_cliente IS NULL OR char_length(observacao_cliente) <= 1200);
  END IF;
END;
$$;
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'observacao_cliente', NULLIF(btrim(p.observacao_cliente), ''),
